-- Auto-generated from source_attribute_observation.schema.json v0.1.0
-- Configuration: config/env_config.yaml (schema=control)
-- DO NOT EDIT MANUALLY — Regenerate from contract + config

CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.source_attribute_observation (
  ...
  -- Constraints
  CONSTRAINT source_attribute_observation_pk PRIMARY KEY (record_id)
)
USING DELTA
TBLPROPERTIES (
...
ALTER TABLE insurance_source_discovery.control.source_attribute_observation ADD CONSTRAINT source_attribute_observation_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.source_attribute_observation ADD CONSTRAINT source_attribute_observation_constraint_role_check CHECK (constraint_role IN ('PRIMARY_KEY', 'FOREIGN_KEY', 'UNIQUE', 'CHECK', 'NONE'));