...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.review_decision (
...
ALTER TABLE insurance_source_discovery.control.review_decision ADD CONSTRAINT review_decision_lifecycle_state_check CHECK (lifecycle_state IN ('RECORDED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.review_decision ADD CONSTRAINT review_decision_decision_check CHECK (decision IN ('APPROVE', 'REJECT', 'REVISE', 'DEFER'));