-- Auto-generated from skill_resolution.schema.json v0.1.0
-- Configuration: config/env_config.yaml (schema=control)
-- DO NOT EDIT MANUALLY — Regenerate from contract + config

CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.skill_resolution (
  ...
  -- Constraints
  CONSTRAINT skill_resolution_pk PRIMARY KEY (record_id)
)
USING DELTA
TBLPROPERTIES (
...
ALTER TABLE insurance_source_discovery.control.skill_resolution ADD CONSTRAINT skill_resolution_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));