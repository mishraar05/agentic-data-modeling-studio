-- Auto-generated from review_decision.schema.json vunknown
...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.gw_pc_bronze.review_decision (
  ...
  CONSTRAINT lifecycle_state_check CHECK (lifecycle_state IN ('RECORDED', 'SUPERSEDED')),
  CONSTRAINT decision_check CHECK (decision IN ('APPROVE', 'REJECT', 'REVISE', 'DEFER'))
)
USING DELTA
TBLPROPERTIES (
