...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.source_attribute_observation ADD CONSTRAINT source_attribute_observation_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.source_attribute_observation ADD CONSTRAINT source_attribute_observation_constraint_role_check CHECK (constraint_role IN ('PRIMARY_KEY', 'FOREIGN_KEY', 'UNIQUE', 'CHECK', 'NONE'));