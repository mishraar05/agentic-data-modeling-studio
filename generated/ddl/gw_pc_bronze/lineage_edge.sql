...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.lineage_edge ADD CONSTRAINT lineage_edge_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.lineage_edge ADD CONSTRAINT lineage_edge_source_type_check CHECK (source_type IN ('SOURCE_ATTRIBUTE', 'EVIDENCE_ITEM', 'ARTIFACT'));