-- Auto-generated from analytical_requirement.schema.json v0.1.0
-- Configuration: config/env_config.yaml (schema=gw_pc_bronze)
-- DO NOT EDIT MANUALLY — Regenerate from contract + config

CREATE TABLE IF NOT EXISTS insurance_source_discovery.gw_pc_bronze.analytical_requirement (
  ...
  -- Constraints
  CONSTRAINT analytical_requirement_pk PRIMARY KEY (record_id)
)
USING DELTA
TBLPROPERTIES (
...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.analytical_requirement ADD CONSTRAINT analytical_requirement_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.analytical_requirement ADD CONSTRAINT analytical_requirement_priority_check CHECK (priority IN ('MUST', 'SHOULD', 'NICE_TO_HAVE'));