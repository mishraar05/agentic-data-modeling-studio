...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.gw_pc_bronze.business_rule (
...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.business_rule ADD CONSTRAINT business_rule_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.business_rule ADD CONSTRAINT business_rule_rule_type_check CHECK (rule_type IN ('VALIDATION', 'CALCULATION', 'TRANSFORMATION', 'DERIVATION'));