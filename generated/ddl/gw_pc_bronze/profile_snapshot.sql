...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.profile_snapshot ADD CONSTRAINT profile_snapshot_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.profile_snapshot ADD CONSTRAINT profile_snapshot_profiling_mode_check CHECK (profiling_mode IN ('FULL', 'METADATA_ONLY', 'RESTRICTED'));