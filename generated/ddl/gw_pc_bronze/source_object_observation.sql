...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.source_object_observation ADD CONSTRAINT source_object_observation_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.source_object_observation ADD CONSTRAINT source_object_observation_object_type_check CHECK (object_type IN ('TABLE', 'VIEW', 'MATERIALIZED_VIEW'));