...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.artifact_version ADD CONSTRAINT artifact_version_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.artifact_version ADD CONSTRAINT artifact_version_artifact_type_check CHECK (artifact_type IN ('SDD_OBJECT', 'SDD_ATTRIBUTE', 'SDD_RELATIONSHIP', 'SDD_CODE_VALUE', 'SDD_HANDOFF'));