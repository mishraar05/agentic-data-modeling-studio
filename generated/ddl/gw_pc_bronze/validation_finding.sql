-- Auto-generated from validation_finding.schema.json vunknown
...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.gw_pc_bronze.validation_finding (
  ...
  CONSTRAINT finding_type_check CHECK (finding_type IN ('SCHEMA', 'REFERENTIAL', 'COVERAGE', 'POLICY', 'CONTRADICTION')),
  CONSTRAINT severity_check CHECK (severity IN ('BLOCKING', 'ERROR', 'WARNING', 'INFO'))
)
USING DELTA
TBLPROPERTIES (
