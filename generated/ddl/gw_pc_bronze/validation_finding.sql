...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.gw_pc_bronze.validation_finding (
...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.validation_finding ADD CONSTRAINT validation_finding_lifecycle_state_check CHECK (lifecycle_state IN ('OPEN', 'RESOLVED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.validation_finding ADD CONSTRAINT validation_finding_finding_type_check CHECK (finding_type IN ('SCHEMA', 'REFERENTIAL', 'COVERAGE', 'POLICY', 'CONTRADICTION'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.validation_finding ADD CONSTRAINT validation_finding_severity_check CHECK (severity IN ('BLOCKING', 'ERROR', 'WARNING', 'INFO'));