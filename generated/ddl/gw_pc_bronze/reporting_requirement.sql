...
ALTER TABLE insurance_source_discovery.gw_pc_bronze.reporting_requirement ADD CONSTRAINT reporting_requirement_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.gw_pc_bronze.reporting_requirement ADD CONSTRAINT reporting_requirement_priority_check CHECK (priority IN ('MUST', 'SHOULD', 'NICE_TO_HAVE'));