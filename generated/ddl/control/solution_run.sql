...
ALTER TABLE insurance_source_discovery.control.solution_run ADD CONSTRAINT solution_run_workflow_state_check CHECK (workflow_state IN ('VALIDATED', 'METADATA_READY', 'PROFILE_READY', 'EVIDENCE_READY', 'CONTEXT_READY', 'SDD_READY', 'SILVER_READY', 'GOLD_READY', 'STTM_READY', 'PUBLISHED'));
ALTER TABLE insurance_source_discovery.control.solution_run ADD CONSTRAINT solution_run_profiling_policy_check CHECK (profiling_policy IN ('FULL', 'METADATA_ONLY', 'RESTRICTED'));
