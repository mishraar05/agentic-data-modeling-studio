...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.open_question (
...
ALTER TABLE insurance_source_discovery.control.open_question ADD CONSTRAINT open_question_lifecycle_state_check CHECK (lifecycle_state IN ('OPEN', 'RESOLVED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.open_question ADD CONSTRAINT open_question_question_type_check CHECK (question_type IN ('MISSING_EVIDENCE', 'AMBIGUOUS_MEANING', 'CONFLICTING_EVIDENCE', 'UNCLEAR_REQUIREMENT'));