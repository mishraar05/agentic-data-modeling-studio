...
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_relationship_type_check CHECK (relationship_type IN ('FOREIGN_KEY', 'INFERRED_FK', 'LOOKUP', 'BRIDGE', 'SELF_REFERENCE'));
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_critic_status_check CHECK (critic_status IN ('CONFIRMED', 'CONTESTED', 'NOT_ASSESSED'));