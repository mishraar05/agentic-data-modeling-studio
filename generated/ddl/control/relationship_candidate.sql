-- Auto-generated from relationship_candidate.schema.json v0.1.0
-- Configuration: config/env_config.yaml (schema=control)
-- DO NOT EDIT MANUALLY — Regenerate from contract + config

CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.relationship_candidate (
...
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_lifecycle_state_check CHECK (lifecycle_state IN ('COMMITTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_relationship_type_check CHECK (relationship_type IN ('PRIMARY_KEY', 'FOREIGN_KEY', 'INFERRED_FK', 'LOOKUP'));
ALTER TABLE insurance_source_discovery.control.relationship_candidate ADD CONSTRAINT relationship_candidate_cardinality_check CHECK (cardinality IN ('1:1', '1:M', 'M:M'));