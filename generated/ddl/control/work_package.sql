-- Auto-generated from work_package.schema.json v0.1.0
-- Configuration: config/env_config.yaml (schema=control)
-- DO NOT EDIT MANUALLY — Regenerate from contract + config

CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.work_package (
  ...
  -- Constraints
  CONSTRAINT work_package_pk PRIMARY KEY (record_id)
)
USING DELTA
TBLPROPERTIES (
...
ALTER TABLE insurance_source_discovery.control.work_package ADD CONSTRAINT work_package_lifecycle_state_check CHECK (lifecycle_state IN ('ACTIVE', 'CLOSED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.work_package ADD CONSTRAINT work_package_workflow_state_check CHECK (workflow_state IN ('VALIDATED', 'METADATA_READY', 'PROFILE_READY', 'EVIDENCE_READY', 'CONTEXT_READY', 'SDD_READY', 'SILVER_READY', 'GOLD_READY', 'STTM_READY', 'PUBLISHED'));