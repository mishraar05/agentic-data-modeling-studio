...
CREATE TABLE IF NOT EXISTS insurance_source_discovery.control.analytical_requirement (
...
ALTER TABLE insurance_source_discovery.control.analytical_requirement ADD CONSTRAINT analytical_requirement_lifecycle_state_check CHECK (lifecycle_state IN ('DRAFT', 'APPROVED', 'REJECTED', 'SUPERSEDED'));
ALTER TABLE insurance_source_discovery.control.analytical_requirement ADD CONSTRAINT analytical_requirement_priority_check CHECK (priority IN ('MUST', 'SHOULD', 'NICE_TO_HAVE'));