...
2. **No knowledge pack is currently runtime-eligible, so the agent cannot execute the positive semantic path today.** Phase 0 resolves context through `select_approved_pack`, which fails closed unless a pack is both `APPROVED` and `runtime_eligible: true`. Every pack in `knowledge/registry/pack_registry.yml` (0.3.0–0.6.0) is `approval_state: CANDIDATE` with `runtime_eligible: false`, so Phase 0 fails closed on all of them and Phases 3–5 have no governed glossary or domain code sets to reason against. This is a governance action (a human must approve a pack), not something the agent may do for itself. The structural `evidence_state` vocabulary is contract-owned and therefore does not depend on pack promotion; domain code sets and any pack-owned handling vocabulary still require an approved, pinned pack version.
